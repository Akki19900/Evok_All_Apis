...
@Service
public class QrTxnStatusByExtTxnIdServiceImpl implements QrTxnStatusByExtTxnIdService {
    ...
    private static String generateexnStatusRrnChecksum(JSONObject qrObject, String checkSumKey) {
    ...
    @Override
    public String statusByRrn(QrTxnStatusByExtTxnId txnStatusRrn) {
        JSONObject obj = getJsonRequest();

        ENC_KEY = txnStatusRrn.getEncKey();

        obj.put("source", txnStatusRrn.getSource());
        obj.put("channel", txnStatusRrn.getChannel());
        obj.put("terminalId", txnStatusRrn.getTerminalId());
        obj.put("extTransactionId", txnStatusRrn.getExtTransactionId());
        // System.out.println("Raw Request" + obj.toString());
        String checksum = generateexnStatusRrnChecksum(obj, txnStatusRrn.getChecksum());
        System.out.println("Checksum is " + checksum);
        obj.put("checksum", checksum);
        // System.out.println("Final string to encrypt is " + obj.toString());
        String encryptedReq = encryptRequest(obj.toString(), txnStatusRrn.getEncKey());
